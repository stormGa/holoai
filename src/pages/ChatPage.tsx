    ...
    <div className="flex h-full bg-white">
      ...
      <div className="w-64 border-r border-gray-200 flex flex-col bg-gray-50/50">
        <div className="p-4 border-b border-gray-200 flex items-center justify-between">
          <h2 className="font-semibold text-gray-700 flex items-center gap-2"><Clock size={18} /> 历史记录</h2>
          <button
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1">
          {sessions.map((session) => (
            <button
              ...
              className={`w-full text-left p-3 rounded-lg transition-all group ${currentSessionId === session.id ? 'bg-white shadow-sm ring-1 ring-gray-200' : 'hover:bg-white hover:shadow-sm'}`}
            >
              <div className="flex items-center gap-3 mb-1">
                <MessageSquare size={14} className={`transition-colors ${currentSessionId === session.id ? 'text-blue-500' : 'text-gray-400 group-hover:text-blue-500'}`} />
                <span className={`text-sm font-medium truncate ${currentSessionId === session.id ? 'text-gray-900' : 'text-gray-700'}`}>{session.title}</span>
              </div>
              <p className="text-xs text-gray-400 pl-6.5">{session.date}</p>
            </button>
          ))}
        </div>
      </div>
              ...
              {messages.map((msg) => (
                <div key={msg.id} className={`flex w-full ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`flex gap-4 items-start max-w-2xl ${msg.sender === 'user' ? 'flex-row-reverse' : ''}`}>
                    ...
                    <div className={`p-3 rounded-lg ${msg.sender === 'user' ? 'bg-blue-600 text-white' : 'bg-gray-100'}`}>
        ...
        <div className="p-4 bg-white border-t border-gray-200 flex-shrink-0">
          <div className="relative max-w-4xl mx-auto">
            <form onSubmit={handleSendMessage}>
              <textarea value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleSendMessage(e); } }} placeholder="在这里输入您的问题..." className="w-full min-h-[56px] p-4 pl-14 pr-16 text-base rounded-2xl border-2 border-gray-200 bg-gray-50 shadow-sm focus:border-blue-500 focus:ring-4 focus:ring-blue-50 resize-none transition-all" rows={1} />
