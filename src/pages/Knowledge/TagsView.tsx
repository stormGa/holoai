            ...
            <div className="bg-white/50 backdrop-blur-sm p-6 rounded-3xl border border-white/60 shadow-sm">
                ...
                <div className="flex items-center justify-between mb-8 px-2">
                    <div className="flex items-center gap-3">
                        <h2 className="text-xl font-bold text-gray-800">全域话题概览</h2>
                        <span className="px-2 py-0.5 rounded-full bg-blue-100 text-blue-700 text-xs font-semibold">20 个主要领域</span>
                    </div>
                    <div className="flex gap-2">
                        <button className="text-sm text-gray-500 hover:text-gray-800 transition-colors">按热度排序</button>
                        <span className="text-gray-300">|</span>
                        <button className="text-sm text-gray-500 hover:text-gray-800 transition-colors">按时间排序</button>
                    </div>
                </div>
                ...
                <div className="columns-2 md:columns-3 lg:columns-4 xl:columns-5 gap-4 space-y-4">
                    {topics.map(topic => (
                        <div key={topic.id} className="break-inside-avoid relative group cursor-pointer">
                            <div className="absolute inset-0 bg-white/40 backdrop-blur-md rounded-2xl shadow-sm border border-white/50 transition-all duration-300 group-hover:scale-[1.02] group-hover:shadow-md group-hover:border-blue-200/50"></div>
                            <div className="relative p-5 flex flex-col h-full bg-gradient-to-br from-white/80 to-transparent rounded-2xl">
                                ...
                                <div className="flex items-start justify-between mb-3">
                                    ...
                                    <div className={`size-2 rounded-full ${topic.count > 100 ? 'bg-red-500 animate-pulse' : 'bg-gray-200'}`}></div>
                                </div>

                                <div className="mt-auto">
                                    <h3 className="font-bold text-gray-800 text-[15px]">{topic.label}</h3>
                                    <div className="text-[10px] font-medium px-1.5 py-0.5 rounded-md bg-gray-100 text-gray-500 inline-block mt-1">
                                        {topic.count} 个知识点
                                    </div>
                                </div>
                            </div>
                ...
                <div className="columns-1 sm:columns-2 md:columns-3 lg:columns-4 gap-4 space-y-4">
                    {topics.map(topic => (
                        <div key={`real-${topic.id}`} className="break-inside-avoid group relative cursor-pointer">
                            <div className="absolute inset-0 bg-white rounded-xl shadow-sm border border-gray-100 transition-all duration-300 group-hover:-translate-y-1 group-hover:shadow-lg group-hover:border-blue-200"></div>
                            ...
                            <div className="relative p-4 z-10">
                                <div className="flex items-center justify-between mb-3">
                                    ...
                                    <span className="text-[10px] bg-gray-50 text-gray-400 px-1.5 py-0.5 rounded border border-gray-100">#{topic.id}</span>
                                </div>
                ...
                <div className="columns-1 sm:columns-2 md:columns-3 lg:columns-4 gap-5 space-y-5">
                    {topics.map(topic => (
                        <div key={topic.id} className="break-inside-avoid relative group cursor-pointer transition-all duration-300 hover:-translate-y-1">
                            <div className="bg-white rounded-2xl p-5 shadow-[0_2px_8px_rgba(0,0,0,0.04)] border border-gray-100 hover:shadow-[0_8px_24px_rgba(0,0,0,0.08)] hover:border-blue-100 transition-all h-full">
                                ...
                                <div className="flex items-start justify-between mb-4">
                                    ...
                                    <div className={`px-2 py-0.5 rounded-full text-[10px] font-bold border ${topic.count > 80 ? 'bg-red-50 text-red-600 border-red-100' : 'bg-gray-50 text-gray-500 border-gray-100'}`}>
                                ...
                                <div className="space-y-2 mt-4 pt-4 border-t border-gray-100/50">
                                    <div className="flex items-center justify-between text-xs text-gray-500">
                                        ...
                                        <span className="font-semibold text-gray-700">{topic.conversationCount}</span>
                                    </div>
                                    <div className="flex items-center justify-between text-xs text-gray-500">
                                        <span className="flex items-center gap-1.5"><FileText size={12} className="text-purple-400" /> 文档</span>
                                        <span className="font-semibold text-gray-700">{topic.documentCount}</span>
                                    </div>
                                    <div className="flex items-center justify-between text-xs text-gray-500">
                                        <span className="flex items-center gap-1.5"><Zap size={12} className="text-amber-400" /> 信息流</span>
                                        <span className="font-semibold text-gray-700">{topic.streamCount}</span>
                                    </div>
                                </div>
